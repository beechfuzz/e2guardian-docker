...
Basically, if the UID/GID of the user that runs the E2Guardian daemon inside the container doesn't match the UID/GID of the owner doesn't have  `PUID` and `PGID` variables will change the UID and GID of the e2guardian user account inside the container.  The default in this container is  `PUID=1000`  and  `PGID=1000`.  To find yours, use the  `id user`  command in your host.  Example:

    $ id dockeruser
     uid=1011(dockeruser) gid=1011(dockeruser) groups=1011(dockeruser)

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMjYwNzQyMTEsNzE4MDU3ODYwXX0=
-->